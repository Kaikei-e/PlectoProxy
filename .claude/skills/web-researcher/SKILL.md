...
- Plecto は仕様変動の速い領域（WASI 0.3 / Component Model / `wasm32-wasip3`）に立脚するので、
  **取得日と「確定事実 / 現時点の計画（projected）」の区別**を常に明示する
...
1. **公式サイトを特定する**
   ...
   - 例: Rust → `doc.rust-lang.org`, wasmtime → `docs.wasmtime.dev`, Component Model/WIT →
     `component-model.bytecodealliance.org`, WASI → `wasi.dev` / WASI repos, Anthropic skills →
     `docs.claude.com`
...
3. **GitHub リポジトリも公式扱いする**
   ...
   - Bytecode Alliance / WASI の WIT 仕様・proposal リポジトリは一次ソース
...
1. **Phase 1 の結果を評価する** — 何がわかったか、何がまだ不明か、を整理する。不明点がなければスキップ
2. **コミュニティソースを検索する** — `WebSearch` でドメイン制限なしの広範検索。必要なら複数クエリ
3. **情報源の信頼性でフィルタする**

   | Tier | ソース種別 | 扱い方 |
   |------|-----------|--------|
   | S | 公式ドキュメント、RFC/proposal、公式ブログ | そのまま採用 |
...
4. **有望なページを WebFetch で深読みする** — Tier A-B から 2-3 件。公式と矛盾しないか確認

### Agent による並列取得

複数の URL を WebFetch する場合、Agent（`subagent_type=Explore` 等）を使って並列化できる。
ただし、5 件以上の並列取得は避ける。質が落ちる。

## Phase 3: 深掘り調査 (deep のみ)

**目標:** エッジケース、既知の問題、代替案を洗い出す。`--depth=shallow` の場合はスキップ。

1. **既知の問題・落とし穴を検索する** — `"<技術名> gotcha/pitfall/common mistakes"`、GitHub Issues `label:bug`
2. **代替技術・アプローチを検索する** — `"<技術名> vs"`, `"<技術名> alternative"`、比較記事からトレードオフ抽出
3. **最新動向を確認する** — リリースノート、ロードマップ、deprecation notice、破壊的変更予定
...
- 調査日: <date>
- 最も古いソース: <date/URL>
- 確定事実 / projected の区別: <仕様変動領域なら必ず明記>
...
- URL は必ず含める。Tier 評価で読者が信頼度を即座に判断できるようにする

## 守ること

- 検索前に目的を明文化する／公式ドキュメントを必ず最初に当たる
- 情報源の信頼性を常に意識する／古い情報には日付を明記して警告する
- 出典のない主張は書かない／検索結果が不十分でも捏造しない
- レポートは簡潔に。10 件以上並べるより 3-5 件の高信号ソースを深く読む
