...
description: >-
  ...
  header bytes (list<u8>), the filter / filter-body world split (header-only vs buffered
  body), versioning/compat with frozen v0.1/v0.2 load-time adapters, and the projected
  stream<u8> + wasm32-wasip2 migration.
when_to_use: >-
...
2. **独自ワールド（Fork 2）— 現行は zero-WASI・自前 http 型（ADR 000010）.** 現行契約は
   ...
   型安定を gate とする projected（ADR 000025）。`wasi:http/middleware`（コンポーネント＝完全な
...
6. **header-only と body-transform を契約レベルで分離（Fork 6・実装済み）.** 「ボディ非接触」は
   base `filter` world における `on-request-body` export の**不在**で構造的に表現し、host は body を
   ゼロコピー素通しする（ADR 000038）。ボディ変換は `filter-body` world の buffered `list<u8>`
   （buffer-then-decide、ADR 000025）。`stream<u8>` での true-streaming 化は後続増分の projected。
...
- 現行ビルドは **zero-WASI の `wasm32-unknown-unknown`**（ADR 000010、wit-component で component 化）。
  buffered `list<u8>` の body v1 も WASI/async 不要でこのターゲットのまま（ADR 000025）。
  `wasm32-wasip2` への移行は `stream<u8>` true-streaming 増分から。P3 WIT はその先（`wasm32-wasip3`
  は rustc 正式ターゲット未満、ゲスト toolchain 対応進行中）。確定したら追従する。
