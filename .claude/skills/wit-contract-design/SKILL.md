...
description: |
  Design and evolve Plecto's `plecto:filter` WIT world and host-API surface — the type contract
  between the native fast path and untrusted WASM filters. Covers worlds/interfaces, resources,
  stream<u8> bodies, the decision/short-circuit variant, minimal-capability host-API slicing,
  reusing wasi:http types, versioning/compat, and the wasm32-wasip2→P3 migration.
  Use when authoring or changing .wit files, designing the filter contract or host-API, or the
  user mentions WIT / Component Model / world / 「契約を設計」「WIT を書く/直す」.
user-invocable: true
...
2. **独自ワールド + wasi:http 型の再利用（Fork 2）.** `plecto:filter` ワールドを定義し、内部で
   `wasi:http` の request/response 型を再利用する。`wasi:http/middleware`（コンポーネント＝完全な
   HTTP ハンドラ）は粒度が違うので採らない。これで polyglot 互換を保ちつつ Plecto の語彙
   （decision / hooks / host-API）を持てる。
3. **判断は variant で（Tenet 3）.** 戻り値は `decision` variant: `continue` / `modified` /
   `short-circuit`。曖昧なフラグや暗黙の副作用にしない。将来の追加に備え host 側 match は網羅的に。
...
- 当面は **`wasm32-wasip2` でビルドしつつ P3 WIT をターゲット**する二段構え（`wasm32-wasip3` は
