...
- **M2 — データ経路（fast path）** 🚧 *(slice 1 着地)*
  **着地（slice 1）:** `plecto-server` crate —— tokio + hyper の **HTTP/1.1** listener。各リクエストを host＋path-prefix で route 照合し、その route の filter chain を `spawn_blocking` ブリッジ経由で M1 の trusted プールに載せて駆動（wasmtime の `!Send` Store は `.await` を跨がない）、host-native な prefix strip を適用し、route の upstream（hyper-util pooling client）へ転送、ボディは opaque にストリーム透過。*Plecto はこれで実際のリバースプロキシになった。* **保留（後続スライス）:** TLS 終端（rustls）、HTTP/2（h2）→ HTTP/3（quinn）、インスタンス間ロードバランシング & upstream health。— [ADR 12](docs/ADR/000012.md) · [13](docs/ADR/000013.md)
- **M3 — async & ボディ** *(2段トリガ)*
