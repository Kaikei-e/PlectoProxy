...
| **Rate limiting** | native L7 token-bucket の床（**route** 単位 / **client-IP** 単位）。加えてフィルタに貸す per-filter の `host-ratelimit`。**ノードローカル**（マルチレプリカでの扱いは [hardening ガイド](docs/hardening.ja.md) 参照） — [33](docs/ADR/000033.md) · [53](docs/ADR/000053.md) |
...
| **M6** — polyglot SDK & リファレンスフィルタ | 🚧 例が着地 | MoonBit/JS/C の例フィルタ + 共通 conformance スイート（CI ゲート）。SSRF ガード付き outbound HTTP（feature-gated）。SDK と Go（最小 WASI リンカ待ち）は未着手 |
...
Plecto は重要な設計判断をすべて ADR に、Fork 形式（*判断 / 根拠 / 再検討条件*）で記録している。54 本（accepted 53・proposed 1）すべては [`docs/ADR/`](docs/ADR/) にあり、起点は [ADR 000001](docs/ADR/000001.md)（相補的な二つの構成要素）。各 ADR は土台にした判断へ相互リンクしている。
