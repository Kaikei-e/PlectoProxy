...
- **M2 — The data path (fast path)** 🚧 *(slice 1 landed)*
  **Landed (slice 1):** a `plecto-server` crate — a tokio + hyper **HTTP/1.1** listener that routes each request by host + path prefix, runs that route's filter chain via a `spawn_blocking` bridge to the M1 trusted pool (wasmtime's `!Send` Store never crosses `.await`), applies a host-native prefix strip, and forwards to the route's upstream (hyper-util pooling client), streaming bodies opaquely. *Plecto is now an actual reverse proxy.* **Pending (next slices):** TLS termination (rustls), HTTP/2 (h2) → HTTP/3 (quinn), inter-instance load balancing & upstream health. — [ADR 12](docs/ADR/000012.md) · [13](docs/ADR/000013.md)
