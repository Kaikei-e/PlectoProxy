...
各シナリオの subject / generator / 出力 CSV / 読むべき invariant / 対応 ADR / tier。
tier の定義（T0 quick / T1 gate / T2 report / T3 deep）と「どの変更でどの tier を回すか」の
決定表は [`methodology.md`](methodology.md) § Measurement tiers。窓長は `run-perf.sh` 冒頭の
TIER 表（`TIER=gate|report`）1 箇所で管理する。

### Smoke & gate（T0 / T1）

| phase | tier | subject | generator | 出力 | invariant |
| --- | --- | --- | --- | --- | --- |
| `quick` | T0 | ceiling 1 点 + idle RSS の smoke check（~1 分、k6/Docker 不要） | oha | （非追跡・console のみ） | 動作確認のみ |
| `gate` | T1 | **変更ごとの回帰ゲート**（~6–7 分）: dispatch floor / apikey cost の A/B/C interleave ×3、固定 2,000 rps tail（+ `resp-ctx`）、rate-limit tax ×2、enforcement 10 s、RR、圧縮 ejection 40 s | oha + k6 + loadgen | `gate.csv`（invariant, value, ci_half, band, verdict） | **機械判定**: 全 invariant が [`perf/gate_tolerances.toml`](perf/gate_tolerances.toml) の帯内 → exit 0 |
| `industry` | T2 束 | RFC 9411 形のコア KPI 束: RR/CRR ceiling + open-loop latency + mix | oha + loadgen + k6 | `ceiling.csv`, `openloop.json`, `mix.csv` | 方法論の最短回帰。詳細は `all` |

### Fast path（native）

| phase | tier | subject | generator | 出力 | invariant |
| --- | --- | --- | --- | --- | --- |
| `ceiling` | T2 | plain HTTP/1.1: keep-alive **RR** + cold-connection **CRR** | oha（`--disable-keepalive`） | `ceiling.csv`（`kpi` 列） | connection 再利用の価値。`wasm` / `tls` はこの数値を再利用 |
| `sweep` | T2 | throughput/latency vs concurrency | k6 constant-vus（50–800 VU） | `sweep.csv` | 曲線形（plateau→graceful decline、cliff 無し） |
| `openloop` | T2 | coordinated-omission-safe tail（既定: schedule-latency） | **`plecto-loadgen openloop`**（`OPENLOOP_GEN=k6` 可） | `openloop.json` + `openloop_hist.csv`（HDR 分布ダンプ） | 飽和点での tail。rate は sweep peak の 70% または `OPENLOOP_RATE` |
| `rr` | T1/T2 | round-robin の均等性 | `plecto-loadgen rr`（X-Instance 集計） | `rr.csv` | 1 req 精度で 1/3 ずつ |
| `ejection` | T1（40s 圧縮）/ T2（75s） | health-eject + fail-closed（ADR 000017） | `plecto-loadgen ejection`（`--toggle-at` でタイムライン指定可） | `ejection_*.csv` | ~1s 時定数・503 fail-closed・~1s 復帰 |
| `swap` | T2 | endpoint-set の swap under load（ADR 000044、reload が health ではなくアドレス集合自体を変える） | `plecto-loadgen swap`（`--exec-at` で manifest 書換 + SIGHUP を時刻指定実行） | `swap.csv`, `swap_events.csv` | ejection と同じ ~1s 時定数で新集合に追従（同じ `ArcSwap` 差し替え経路） |
| `tls` | T2（resumed ラングは T3・手動） | TLS 終端の分解（h1/keepalive/full handshake/**resumed handshake**/h2、ADR 000052 で三点分解が完成） | oha（keepalive/h2）+ `plecto-loadgen tls --mode full\|resumed`（handshake ラング） | `tls.csv`, `tls_full.csv`, `tls_resumed.csv` | record-layer 安い・handshake 支配・resumed が full と keep-alive の中間に入る。`plain (h1)` 行は `ceiling.csv` を参照、測り直さない。resumed ラングは ADR 000051 の provider A/B と同一計測セッションで取る |
| `footprint` | T2 | idle RSS + bytes/conn | `plecto-loadgen hold` | `footprint.txt` | conn あたり限界バイト |
...
| phase | tier | subject | generator | 出力 | invariant |
| --- | --- | --- | --- | --- | --- |
| `wasm` | T1（interleave 版）/ T2 | filter overhead（baseline/pooled/on-demand）+ short-circuit | oha + k6 | `wasm_overhead.csv`, `wasm_mixed.csv` | µs/req と pooling の価値（init 再払いの差）。`baseline` 行は `ceiling.csv` を参照 |
| `v03` | T3 | v0.3.0 response-context / `replace`（ADR 000073）+ compression opt-in（ADR 000074/075）の行使時コスト | oha | `v03_response*.csv`, `v03_compression*.csv` | noop-pooled→resp-ctx の µs/req（`resp-ctx` tail は gate にも常設）・gzip 1 サイズの回帰床 |
| `ratelimit` | T1（tax/enforce）/ T2 | host token-bucket（ADR 000026）: overhead / enforcement / per-key fairness | k6 | `ratelimit_{overhead,enforce,fairness}.csv` | 許可 rps が refill rate に収束・hot key が light key を starve しない |
| `body` | T2 | request-body hook（ADR 000025）の buffer-then-decide コスト + payload sweep | k6 | `body.csv` | hook の payload 比例コスト・streaming passthrough との対比 |
| `ws` | T2 | WebSocket Upgrade トンネル（ADR 000048）: handshake rate / tunnel footprint / echo throughput | `plecto-loadgen ws`（handshake/hold/echo モード） | `ws_handshake.csv`, `ws_footprint.csv`, `ws_echo.csv` | 双方向 splice が短命リクエストと根本的に異なる負荷特性を持つことを可視化。conn_limit / breaker permit / least-request in-flight のトンネル寿命分の会計 |
...
| phase | tier | subject | generator | 出力 | invariant |
| --- | --- | --- | --- | --- | --- |
| `mix` | T2 | 重み付き request mix（60/25/10/5 read/auth/write/large）+ 同レート read-only 対照 | k6 constant-arrival-rate | `mix.csv` | blend のコストが offered load ではなく traffic 構成に帰属する |
| `h3` | T2（負荷は T3 予定） | HTTP/3 の機能確認のみ（負荷は deferred — oha/k6 に native H3 無し。h2load `--npn-list h3` 等が候補） | curl `--http3-only` | `h3.txt` | status=200 http_version=3 |
...
# phases: quick gate ceiling sweep openloop rr ejection swap wasm v03 tls h3 ws footprint ratelimit body mix industry all
bash bench/perf/run-perf.sh all      # T2 report（`just report`）。BACKEND_LATENCY_MS=0 の
                                     # bench-server 系 4 phase は 1 起動を共有する
# 変更ごとの回帰ゲート（T1、機械判定。band: bench/perf/gate_tolerances.toml）
bash bench/perf/run-perf.sh gate     # `just gate`。exit 0 = 全 invariant が帯内
...
- **2026-07-11: 「レポート」と「ゲート」の分離（tier 再編）。** `all` が「リリース snapshot 生成」と
  「変更ごとの回帰検出」を兼ねて実質回されない問題を、T1 `gate`（invariant のみ・interleave 反復・
  `gate_tolerances.toml` 帯照合・機械判定、~6–7 分）の新設で解消。窓長は TIER 表 1 箇所に集約し、
  report tier は統計的に過剰だった単発長窓を短縮（sweep/wasm 60→30 s、tail 30→20 s 等。p99 の
  サンプリング誤差は 10 s で十分小さく、残る run-to-run 変動は窓延長では消えない——根拠は
  `methodology.md` § Measurement tiers）。`all` 内で同一 env の bench-server 4 起動
  （footprint/ceiling/ws/wasm）を 1 起動共有に統合。あわせて `plecto-loadgen openloop` を
  HdrHistogram 化（`--hist-out` で分布全体をダンプ）、micro 層に命令数ベンチ
  （gungraun、`--features instruction-bench`）を criterion と並設。
...
- 非追跡（regenerable working data、ルート `.gitignore` の `performance/data/*.csv`）: 各シナリオの CSV・
  ...
  `openloop_hist.csv`（HDR 分布ダンプ）もここ——追跡されるのは判定の**帯**
  （`perf/gate_tolerances.toml`）であって、ホスト依存の測定値ではない。
