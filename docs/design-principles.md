...
A new host-API is cut as "1 interface = 1 capability", preserving deny-by-default. Dangerous capabilities are **quarantined behind off-by-default feature gates before** they land — current instances: `outbound-http` (outside the default build until the wasi:http convergence gate), `outbound-tcp` (the raw-TCP consult capability behind the same SSRF floor, ADR 000060), `streaming-body` (until wasip3 Tier 2), `polyglot-conformance` (no effect on default `cargo test`), `fat-guest` (the minimal-WASI grant for Tier B guests, ADR 000063 — off by default, and even on, inert unless a filter's manifest entry declares `wasi = "minimal"`). The two outbound gates plus `fat-guest` are exactly what the **capabilities** release profile compiles in (ADR 000079) — and compiling one in still grants nothing until a manifest declares it for a specific filter. Shapes that let a filter relax its own constraints by self-declaration (guest-specified bucket capacity, etc.) are forbidden at design time.
