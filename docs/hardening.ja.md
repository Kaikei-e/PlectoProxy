...
Valkey 等）に問い合わせ、実際のフリート全体の上限を保持する。これは Envoy Gateway が同じ課題に対して
文書化している local + global 併用パターンと同型——local がバーストを吸収してから global が実数を保つ。
...
経路は、貸与された outbound capability 経由で外部ストアを叩く**フィルタ**であり、Envoy が external
global rate limit service に採る配置と同型——しかも Plecto ではその「サービス」自体が filter になる
