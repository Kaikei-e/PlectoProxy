...
Plecto をフリート（複数レプリカ + 前段 LB）で動かすためのガイド。前段ロードバランサが依拠できる
...
| nginx / HAProxy のアクティブチェック | チェックの `interval × fall` 以上（nginx はさらに `fail_timeout` を考慮）。 |
| Envoy | `health_checks.interval × unhealthy_threshold` 以上。 |
