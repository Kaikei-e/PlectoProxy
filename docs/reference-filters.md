...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `filters/jwt` | `filter-jwt` | 0.1.1 | `plecto:filter@0.3.0` (`filter`) | `wasm32-wasip2` | `wasi:http` (outgoing-handler, types) + the `wasi:io` / `wasi:cli` slices the wasip2 target bootstraps — outgoing calls happen only on the JWKS-at-init path; the static PEM/JWK path never calls out | **capabilities** | `isolation = "trusted"` (ADR 000070); `outbound-http` allowlist for the JWKS path, `allow = []` for static keys |
