...
**Operator key（運用鍵）**:
運用者が管理する本番署名鍵。公開半分が本番 manifest の `[trust].keys` に載り、秘密半分は CI/KMS 側に留まる。
`plecto package` が PKCS8 PEM として受け取り署名する対象で、dev key と違いプロジェクトが生成も永続化もしない。
_Avoid_: dev key（`plecto dev` 専用鍵と混同）, trust key（公開半分＝trust root 側だけを指す語）

**Config file indirection（設定のファイル間接参照）**:
`[filter.config_files]` — `[filter.config]` と同じキー空間だが、値は**パス**で、ロード/リロード時に読んだ
ファイル内容（UTF-8・両端 trim・上限付き）がそのキーの値として `host-config::get` から供給される。コンテナの
シークレットがファイルで届く形（`/run/secrets`）に合わせ、checked-in な manifest に秘密を実体化させない。
同一キーの二重宣言・欠落・非 UTF-8 は fail-closed。
_Avoid_: env 展開（環境変数由来と混同）, secret store（外部ストア連携の含意——これは単なるファイル読み）
