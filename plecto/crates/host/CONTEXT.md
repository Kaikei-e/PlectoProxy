...
**Outbound capability（outbound TCP）**:
フィルタが外部へ能動 TCP 接続を張る能力（Redis / memcached 等の外部ストアへの consult、ADR 000060）。
標準語彙 `wasi:sockets`（network / instance-network / tcp-create-socket / tcp ＋ ホスト検問付き
ip-name-lookup）で貸し、**UDP と listen（受動 open）は貸さない**（interface 自体をリンクしない）。
既定では**貸さない**。貸す瞬間に outbound HTTP と同一の allowlist ＋ SSRF guard（同じ `classify`・
同じ floor）＋ IP pin ＋ 資源境界（per-request の connect 予算、hook 呼び出しの wall-clock deadline）で囲う。
_Avoid_: socket capability（貸与単位が出ない）, raw sockets（UDP / listen を含む含意）

**Persistent connection（跨リクエスト接続）**:
pooled instance が検問済みの outbound TCP 接続を Store 内リソースとして次リクエストでも使い続けること
（Redis filter の connect+AUTH を毎リクエスト払わないための形、ADR 000060）。「フィルタはステートレス
（状態はホスト KV）」が禁じるのは**判断状態（decision state）**であり、検問済み接続は instance recycle が
回収する ephemeral な通信資源——判断状態ではない。
_Avoid_: connection pooling（fast path の upstream 接続管理と混同）

**Outbound allowlist**:
outbound を貸すフィルタごとに operator が宣言する送信先の許可リスト（HTTP は scheme + host + port、
TCP は host + port の exact match）。
...
**IP-pinned connect**:
SSRF guard の要。ホスト自身が DNS 解決して全 A/AAAA を分類し、検査を通った**具体 IP**にだけ接続を許す
（ホスト名で再接続しない）。検査と接続の間の DNS rebinding（TOCTOU）を塞ぐ。outbound HTTP ではホストが
接続まで所有して実現し、TLS の SNI/証明書検証は元のホスト名で行う。outbound TCP では guest が connect を
駆動するため、ホストは「自身が解決した検証済みアドレスの集合（pinned set）」を connect 検問で照合する形で
同じ不変条件を守る（ADR 000060）。
