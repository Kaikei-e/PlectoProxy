...
use plecto_host::{Header, Host, HttpRequest, LoadOptions, LogLine, RequestDecision, RunError};
...
fn trusted_filter_recovers_after_trap() {
    ...
    let filter = host
        .load(
            "filter-hello",
            &component_bytes(),
            LoadOptions::trusted().with_request_deadline_ms(50),
        )
        .unwrap();
...
fn memory_limit_traps_runaway_allocation() {
    ...
    let filter = host
        .load(
            "filter-hello",
            &component_bytes(),
            LoadOptions::untrusted().with_max_memory_bytes(16 << 20),
        )
        .unwrap();
