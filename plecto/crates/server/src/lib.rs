//! plecto-server — the M2 fast path (ADR 000013).
//!
//! A tokio + hyper HTTP/1.1 listener that turns Plecto from a library into an actual reverse
//! proxy. Per request it: builds a header-only `HttpRequest`, asks the control plane which route
...
/// Serve the fast path on an already-bound `listener` until it errors unrecoverably. Each
/// accepted connection is handled on its own task with HTTP/1.1 keep-alive; a per-connection
/// error is logged, not fatal. Bind with `TcpListener::bind` (the caller picks the addr, so a
/// test can use an ephemeral `127.0.0.1:0` and read `local_addr`).
...
/// instead of dropping the socket), so the request handling's errors are mapped to a 502.
async fn handle(
    state: Arc<ServerState>,
    req: Request<Incoming>,
) -> Result<Response<ResponseBody>, Infallible> {
    Ok(match route_and_proxy(state, req).await {
        Ok(resp) => resp,
        Err(e) => {
...
async fn route_and_proxy(
    ...
    let http_req = to_http_request(&parts);
...
fn to_http_request(parts: &hyper::http::request::Parts) -> HttpRequest {
    let path = parts
        .uri
        .path_and_query()
        .map(|pq| pq.as_str().to_string())
        .unwrap_or_else(|| "/".to_string());
    // authority: the Host header (HTTP/1.1), falling back to the URI's authority.
    let authority = parts
        .headers
